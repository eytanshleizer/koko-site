 ...
 * title + fullscreen button, and a body that fills the rest. Mermaid
 * renders client-side from the <pre class="mermaid"> source (Base.astro
 * handles it).
 *
 * Title behavior:
 *   ```mermaid                  → topbar shows "Diagram"
 *   ```mermaid title="Loop"     → topbar shows "Loop"
 *
...
export function remarkMermaid() {
  return (tree: any) => {
    visit(tree, "code", (node: any) => {
      ...
      const title = escapeHtml(parseTitle(node.meta))
      node.type = "html"
      node.value =
